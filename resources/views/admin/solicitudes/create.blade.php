    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.solicitudes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nombre_solicitante') ? 'is-invalid' : '' }}" name="nombre_solicitante_id" id="nombre_solicitante_id" required>
                    @foreach($nombre_solicitantes as $id => $nombre_solicitante)
                        <option value="{{ $id }}" {{ old('nombre_solicitante_id') == $id ? 'selected' : '' }}>{{ $nombre_solicitante }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_solicitud') ? 'is-invalid' : '' }}" type="text" name="fecha_solicitud" id="fecha_solicitud" value="{{ old('fecha_solicitud') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cantidad_solicitud') ? 'is-invalid' : '' }}" type="number" name="cantidad_solicitud" id="cantidad_solicitud" value="{{ old('cantidad_solicitud') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fabricacion_solicitud') ? 'is-invalid' : '' }}" name="fabricacion_solicitud_id" id="fabricacion_solicitud_id" required>
                    @foreach($fabricacion_solicituds as $id => $fabricacion_solicitud)
                        <option value="{{ $id }}" {{ old('fabricacion_solicitud_id') == $id ? 'selected' : '' }}>{{ $fabricacion_solicitud }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('solicitud_unidad') ? 'is-invalid' : '' }}" type="text" name="solicitud_unidad" id="solicitud_unidad" value="{{ old('solicitud_unidad', '') }}" required>
