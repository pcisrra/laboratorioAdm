    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.herramienta.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cantidad') ? 'is-invalid' : '' }}" type="number" name="cantidad" id="cantidad" value="{{ old('cantidad') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('funcionario_asignado') ? 'is-invalid' : '' }}" name="funcionario_asignado_id" id="funcionario_asignado_id" required>
                    @foreach($funcionario_asignados as $id => $funcionario_asignado)
                        <option value="{{ $id }}" {{ old('funcionario_asignado_id') == $id ? 'selected' : '' }}>{{ $funcionario_asignado }}</option>
                    @endforeach
                </select>
