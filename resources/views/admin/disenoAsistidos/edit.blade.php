...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.disenoAsistido.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.diseno-asistidos.update", [$disenoAsistido->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="descripcion">{{ trans('cruds.disenoAsistido.fields.descripcion') }}</label>
                <input class="form-control {{ $errors->has('descripcion') ? 'is-invalid' : '' }}" type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $disenoAsistido->descripcion) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha') ? 'is-invalid' : '' }}" type="text" name="fecha" id="fecha" value="{{ old('fecha', $disenoAsistido->fecha) }}" required>
                @if($errors->has('fecha'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fecha') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cantidad_horas') ? 'is-invalid' : '' }}" type="number" name="cantidad_horas" id="cantidad_horas" value="{{ old('cantidad_horas', $disenoAsistido->cantidad_horas) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('costo') ? 'is-invalid' : '' }}" type="number" name="costo" id="costo" value="{{ old('costo', $disenoAsistido->costo) }}" step="1" required>
                @if($errors->has('costo'))
                    <div class="invalid-feedback">
                        {{ $errors->first('costo') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usuario_asignado') ? 'is-invalid' : '' }}" name="usuario_asignado_id" id="usuario_asignado_id" required>
                    @foreach($usuario_asignados as $id => $usuario_asignado)
                        <option value="{{ $id }}" {{ ($disenoAsistido->usuario_asignado ? $disenoAsistido->usuario_asignado->id : old('usuario_asignado_id')) == $id ? 'selected' : '' }}>{{ $usuario_asignado }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('nombre_cliente') ? 'is-invalid' : '' }}" name="nombre_cliente_id" id="nombre_cliente_id" required>
                    @foreach($nombre_clientes as $id => $nombre_cliente)
                        <option value="{{ $id }}" {{ ($disenoAsistido->nombre_cliente ? $disenoAsistido->nombre_cliente->id : old('nombre_cliente_id')) == $id ? 'selected' : '' }}>{{ $nombre_cliente }}</option>
                    @endforeach
                </select>
