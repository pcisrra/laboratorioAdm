...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.capacitacion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.capacitacions.update", [$capacitacion->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nombre">{{ trans('cruds.capacitacion.fields.nombre') }}</label>
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $capacitacion->nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('fecha_inicio') ? 'is-invalid' : '' }}" type="text" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $capacitacion->fecha_inicio) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('fecha_fin') ? 'is-invalid' : '' }}" type="text" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $capacitacion->fecha_fin) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('funcionario_capacitacion') ? 'is-invalid' : '' }}" name="funcionario_capacitacion_id" id="funcionario_capacitacion_id" required>
                    @foreach($funcionario_capacitacions as $id => $funcionario_capacitacion)
                        <option value="{{ $id }}" {{ ($capacitacion->funcionario_capacitacion ? $capacitacion->funcionario_capacitacion->id : old('funcionario_capacitacion_id')) == $id ? 'selected' : '' }}>{{ $funcionario_capacitacion }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('asistentes') ? 'is-invalid' : '' }}" name="asistentes[]" id="asistentes" multiple required>
                    @foreach($asistentes as $id => $asistentes)
                        <option value="{{ $id }}" {{ (in_array($id, old('asistentes', [])) || $capacitacion->asistentes->contains($id)) ? 'selected' : '' }}>{{ $asistentes }}</option>
                    @endforeach
                </select>
