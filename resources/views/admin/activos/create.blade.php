...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.activo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.activos.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('activo') ? 'is-invalid' : '' }}" type="text" name="activo" id="activo" value="{{ old('activo', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('costo') ? 'is-invalid' : '' }}" type="number" name="costo" id="costo" value="{{ old('costo') }}" step="0.01" required>
                @if($errors->has('costo'))
                    <div class="invalid-feedback">
                        {{ $errors->first('costo') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('localizacion') ? 'is-invalid' : '' }}" name="localizacion_id" id="localizacion_id" required>
                    @foreach($localizacions as $id => $localizacion)
                        <option value="{{ $id }}" {{ old('localizacion_id') == $id ? 'selected' : '' }}>{{ $localizacion }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ubicacion') ? 'is-invalid' : '' }}" type="text" name="ubicacion" id="ubicacion" value="{{ old('ubicacion', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('func_asignado') ? 'is-invalid' : '' }}" name="func_asignado_id" id="func_asignado_id" required>
                    @foreach($func_asignados as $id => $func_asignado)
                        <option value="{{ $id }}" {{ old('func_asignado_id') == $id ? 'selected' : '' }}>{{ $func_asignado }}</option>
                    @endforeach
                </select>
