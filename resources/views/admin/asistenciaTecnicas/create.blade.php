...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.asistenciaTecnica.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.asistencia-tecnicas.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="detalle">{{ trans('cruds.asistenciaTecnica.fields.detalle') }}</label>
                <input class="form-control {{ $errors->has('detalle') ? 'is-invalid' : '' }}" type="text" name="detalle" id="detalle" value="{{ old('detalle', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('funcionario_asistencia') ? 'is-invalid' : '' }}" name="funcionario_asistencia_id" id="funcionario_asistencia_id" required>
                    @foreach($funcionario_asistencias as $id => $funcionario_asistencia)
                        <option value="{{ $id }}" {{ old('funcionario_asistencia_id') == $id ? 'selected' : '' }}>{{ $funcionario_asistencia }}</option>
                    @endforeach
                </select>
