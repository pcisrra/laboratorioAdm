...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.ingresoMateriale.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ingreso-materiales.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('codigo_material') ? 'is-invalid' : '' }}" name="codigo_material_id" id="codigo_material_id" required>
                    @foreach($codigo_materials as $id => $codigo_material)
                        <option value="{{ $id }}" {{ old('codigo_material_id') == $id ? 'selected' : '' }}>{{ $codigo_material }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('unidad_ingreso') ? 'is-invalid' : '' }}" type="text" name="unidad_ingreso" id="unidad_ingreso" value="{{ old('unidad_ingreso', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('costo_ingreso') ? 'is-invalid' : '' }}" type="number" name="costo_ingreso" id="costo_ingreso" value="{{ old('costo_ingreso') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('usuario_ingreso') ? 'is-invalid' : '' }}" name="usuario_ingreso_id" id="usuario_ingreso_id" required>
                    @foreach($usuario_ingresos as $id => $usuario_ingreso)
                        <option value="{{ $id }}" {{ old('usuario_ingreso_id') == $id ? 'selected' : '' }}>{{ $usuario_ingreso }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('observaciones') ? 'is-invalid' : '' }}" type="text" name="observaciones" id="observaciones" value="{{ old('observaciones', '') }}" required>
