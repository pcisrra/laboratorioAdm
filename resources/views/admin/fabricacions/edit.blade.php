...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fabricacion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fabricacions.update", [$fabricacion->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="proyecto_nombre">{{ trans('cruds.fabricacion.fields.proyecto_nombre') }}</label>
                <input class="form-control {{ $errors->has('proyecto_nombre') ? 'is-invalid' : '' }}" type="text" name="proyecto_nombre" id="proyecto_nombre" value="{{ old('proyecto_nombre', $fabricacion->proyecto_nombre) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('maquina_asignadas') ? 'is-invalid' : '' }}" name="maquina_asignadas[]" id="maquina_asignadas" multiple required>
                    @foreach($maquina_asignadas as $id => $maquina_asignada)
                        <option value="{{ $id }}" {{ (in_array($id, old('maquina_asignadas', [])) || $fabricacion->maquina_asignadas->contains($id)) ? 'selected' : '' }}>{{ $maquina_asignada }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_inicio') ? 'is-invalid' : '' }}" type="text" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $fabricacion->fecha_inicio) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duracion') ? 'is-invalid' : '' }}" type="number" name="duracion" id="duracion" value="{{ old('duracion', $fabricacion->duracion) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tecnico_asignado') ? 'is-invalid' : '' }}" name="tecnico_asignado_id" id="tecnico_asignado_id" required>
                    @foreach($tecnico_asignados as $id => $tecnico_asignado)
                        <option value="{{ $id }}" {{ ($fabricacion->tecnico_asignado ? $fabricacion->tecnico_asignado->id : old('tecnico_asignado_id')) == $id ? 'selected' : '' }}>{{ $tecnico_asignado }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('material_asignados') ? 'is-invalid' : '' }}" name="material_asignados[]" id="material_asignados" multiple required>
                    @foreach($material_asignados as $id => $material_asignado)
                        <option value="{{ $id }}" {{ (in_array($id, old('material_asignados', [])) || $fabricacion->material_asignados->contains($id)) ? 'selected' : '' }}>{{ $material_asignado }}</option>
                    @endforeach
                </select>
